...
-- TRIGGERS
  ...
  -- FRIENDS
-- ANTES DE INSERTAR UNA RELACIÓN SI ES NULL EN STATE, LO ESCRIBE COMO 'Pendiente'
CREATE DEFINER=`root`@`localhost` TRIGGER `friends_BEFORE_INSERT` BEFORE INSERT ON `friends` FOR EACH ROW BEGIN
IF NEW.state IS NULL THEN
	SET NEW.state = 'Pendiente';
END IF;
END
